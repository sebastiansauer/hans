...
### Visits im Zeitverlauf - üro Monat
...
### Visits im Zeitverlauf - pro Woche
...
```{r time2_fingerprint-plot-heatmap}
time2_fingerprint <-
  time_visit_wday_fingerprint |>
  ungroup() |>
  distinct(fingerprint, .keep_all = TRUE) |>
...
```{r time2-heatmap-week-fingerprint}
time2_fingerprint |>
  distinct(fingerprint, .keep_all = TRUE) |>
  ggplot(aes(x = date, y = hour)) +
  geom_bin2d(binwidth = c(7, 1)) + # 1 week, 1 hour
