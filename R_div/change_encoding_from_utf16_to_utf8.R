...
if (length(csv_files) == 0) {
  message(
    "No CSV files found in the specified directory or its subfolders. Please check 'main_folder_path'."
  )
} else {
  message(paste(
    "Found",
    length(csv_files),
    "CSV files to process (including subfolders)."
  ))
  ...
  for (file_path in csv_files) {
    ...
    if (
      !is.null(current_encoding) &&
        (current_encoding == "UTF-16LE" ||
          current_encoding == "UTF-16BE" ||
      ...
      tryCatch(
        {
          data <- readr::read_csv(
            file_path,
            locale = locale(encoding = current_encoding)
          )
      ...
      if (!is.null(data)) {
        ...
        if (overwrite_original) {
          ...
          output_file_path <- file.path(
            file_directory,
            paste0(file_name_parts, output_suffix, ".", file_extension)
          )
          message(paste0(
            "  Saving re-encoded file to: ",
      ...
      if (!is.null(current_encoding)) {
        message(paste0(
          "  File is NOT detected as UTF-16 (detected as '",
          current_encoding,
          "'). Skipping conversion."
        ))
      } else {
        message(
          "  Encoding not reliably guessed as UTF-16. Skipping conversion."
        )
      }
    }
  }
