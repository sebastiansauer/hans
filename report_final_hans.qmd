...
Sehr viele Spalten sind fast leer.
...
#gtsave(tbl_n_action, filename = "tbl_n_action.png")
```
...
```{r time_spent_w_course_university}
...
```{r}
tar_load(n_action_w_date)
#n_action_w_date$month_date_str <- format(n_action_w_date$month_date, "%Y-%m")
```
