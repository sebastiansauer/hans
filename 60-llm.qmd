...
### Token-Länge im Zeitverlauf

```{r prompt_length_date_uni_course_date}
prompt_length_date_uni_course |> 
...
```{r prompt_length_date_uni_course-plot-timeline}
...
```{r n_interactions_w_llm_course_date_course_uni-plot}
n_interactions_w_llm_course_date_course_uni |>
  group_by(floor_date_month) |>
  summarise(n = n()) |>
...
```{r n_interactions_w_llm_course_date_course_uni-plot_with_semesters_shaded}
library(dplyr)
library(lubridate)
library(ggplot2)
...
rect_data_word_per_month <- comp_semester_rects(click_transcript_word_per_month, 
...
rect_data <- comp_semester_rects(ai_transcript_clicks_per_month_count, col_date = "year_month")
