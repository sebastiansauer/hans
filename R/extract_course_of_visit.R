extract_course_role_university_of_visit <- function(d = data_wide_slim){
d |> # WIDE data such as "data_wide_slim"
  select(idvisit,actiondetails_0_url, actiondetails_0_timestamp) |> 
  mutate(course = str_extract(actiondetails_0_url, "(?<=\\.student\\.)[a-zA-Z0-9]+")) |> 
  mutate(university = str_extract(actiondetails_0_url, "(?<=%40)[a-z0-9-]+")) |> 
